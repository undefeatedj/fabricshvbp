...
func signECDSA(k *ecdsa.PrivateKey, digest []byte, opts bccsp.SignerOpts) ([]byte, error) {
	fmt.Println("------------------------------------ SHVBP ECDSA -------------------------------------")
    //读取配置文件 得到n,t
	nt_arr,_:=ReadLineTxt("/Users/jx/nt-config/nt.txt")

	n:=nt_arr[0]
	t:=nt_arr[1]
	tableIndex := generateTableIfNewPriv(k.D,n,t)

	r,s := WBObfECDSASignWithIndex(digest,n,tableIndex)

	s,_,_ = utils.ToLowS(&k.PublicKey,s)
...
/*func verifyECDSA(k *ecdsa.PublicKey, signature, digest []byte, opts bccsp.SignerOpts) (bool, error) {
	...
	//return ecdsa.Verify(k, hash_digest, r, s), nil
}*/
...
func verifyECDSA(k *ecdsa.PublicKey, signature, digest []byte, opts bccsp.SignerOpts) (bool, error) {
	...
	//h := sha256.New()
	//h.Write(digest)
	//hash_digest := h.Sum(nil)
	//
	fmt.Println("------------------------------------ IS VERIFYING -------------------------------------")
	r, s, err := utils.UnmarshalECDSASignature(signature)
	if err != nil {
		return false, fmt.Errorf("Failed unmashalling signature [%s]", err)
	}
